...
@register_dataset("finegym_adaptive_v2")
class FineGymDataset(Dataset):
    ...
    def _load_json_db(self, json_file):
        ...
        if 'train' in self.split and not self.use_full:
            ...
            with open(f'/data3/xiaodan8/actionformer4_1/output/finegym/pred_vit_adaptive_{self.round - 1000}_v2.json') as f:
                pred = json.load(f)
            ...
            # for key in video_list_by_cat:
            #     random.shuffle(video_list_by_cat[key])
            
            # Load sample count from file if it exists
            sample_count_file = '/data3/xiaodan8/actionformer4_1/output/finegym/sample_count.json'
            if os.path.exists(sample_count_file):
                with open(sample_count_file, 'r') as f:
                    self.sample_count = json.load(f)
            else:
                self.sample_count = {'1000': {str(k): 10 for k in range(self.num_classes)}}

            # Initialize sample count for the current round if not present
            if str(self.round) not in self.sample_count:
                self.sample_count[str(self.round)] = {str(k): 0 for k in range(self.num_classes)}

            # assign N annotations per category based on round number
            video_list = []
            for k_item, v_item in video_list_by_cat.items():
                num_samples = self.sample_count[str(self.round - per_round_num)][str(k_item)] + math.ceil(confuse_score[k_item] * per_round_num)
                video_list.extend(circ_slice(v_item, 0, num_samples))
                self.sample_count[str(self.round)][str(k_item)] = num_samples  # Update the sample count for the class

            # Save the updated sample count to file
            from deepspeed import comm as dist
            if not dist.is_initialized() or dist.get_rank() == 0:
                with open(sample_count_file, 'w') as f:
                    json.dump(self.sample_count, f)
