...
from .data_utils import truncate_feats, get_transforms, truncate_video, circ_slice, temporal_iou
...
@register_dataset("finegym_adaptive_v3")
class FineGymDataset(Dataset):
    ...
    def _load_json_db(self, json_file):
        ...
        if 'train' in self.split and not self.use_full:
            ...
            logits_path = f'/data3/xiaodan8/actionformer4_1/output/finegym/logits_adaptive_{prev_round}.pt'
            ...
            with open(f'/data3/xiaodan8/actionformer4_1/output/finegym/pred_vit_adaptive_{prev_round}.json') as f:
                pred = json.load(f)
            with open('/data3/xiaodan8/actionformer4_1/finegym_test_check.json') as f:
                gt_test = json.load(f)
            ...
            video_list = []
            used_vids_dict = {}
            if not os.path.exists('/data3/xiaodan8/actionformer4_1/output/finegym/used_vids_dict.pt'):
                # if used_vids file not exists, use the first 1000 videos in the first round
                num_anno_per_class = 2500 // num_classes_fixed
                ...
                with open('/data3/xiaodan8/actionformer4_1/output/finegym/used_vids_dict.pt', 'r') as f:
            ...
            noise_rounds = 3
            total_samples_target = len(dict_db)  # adjust if different final target
            ...
            with open('/data3/xiaodan8/actionformer4_1/output/finegym/used_vids_dict.pt', 'w') as f:
                json.dump(used_vids_dict, f)
            shuffle(video_list)
            dict_db = tuple(video_list)
            print(f"Total samples target: {total_samples_target}")
            total_filtered = sum(len(selected[cls_idx]['filtered']) for cls_idx in range(self.num_classes))
            print(f"Samples filtered: {total_filtered}")
            print(f'adaptive+margin select: split={self.split} size={len(dict_db)}')

        return dict_db, self.label_dict
