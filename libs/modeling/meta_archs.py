...
class PtTransformer(nn.Module):
    ...
    def __init__(
        ...
        elif backbone_type == 'ActionFormerWithViT':
            self.backbone = make_backbone(
                'ActionFormerWithViT',
                **{
                    'input_dim' : input_dim,
        ...
        elif backbone_type == 'ActionFormerWithViViT':
            self.backbone = make_backbone(
                'ActionFormerWithViViT',
                **{
                    'input_dim' : input_dim,
                    'embd_dim' : embd_dim,
                    'n_head': n_head,
        ...
        elif backbone_type == 'ActionFormerViViTPrompt':
            self.backbone = make_backbone(
                'ActionFormerViViTPrompt',
                **{
                    'input_dim' : input_dim,
                    'embd_dim' : embd_dim,
                    'n_head': n_head,
        ...
        elif backbone_type == 'ActionFormerWithTimesformer':
            self.backbone = make_backbone(
                'ActionFormerWithTimesformer',
                **{
                    'input_dim' : input_dim,
                    'embd_dim' : embd_dim,
                    'n_head': n_head,
        ...
        elif backbone_type == 'ActionFormerWithVideoMAE':
            self.backbone = make_backbone(
                'ActionFormerWithVideoMAE',
                **{
                    'input_dim' : input_dim,
                    'embd_dim' : embd_dim,
                    'n_head': n_head,
        ...
        # 2) Load dataset-level counts and compute class_weights
        if self.active_learning_method == 'adaptive':
            from libs.datasets.finegym_adaptive import class_weights
            # from libs.datasets.thumos_adaptive import class_weights
        else:
            from libs.datasets.finegym_uniform import class_weights
            # from libs.datasets.thumos_uniform import class_weights
    ...
    def losses(
        ...
        if use_focal:
            ...
            if use_weight:
                ...
                weight_normalized = self.class_weights.float().cuda()

                # # calculate frame weight
                # digits = one_hot_to_digits(gt_target)
                # digits = digits[digits != -1]
                # unique_elements, counts = torch.unique(digits, return_counts=True)
                # average_frms = torch.mean(counts.float())
                # counts_normalized_reverse = 1 / (counts / average_frms)
                # frame_weight = {i:1 for i in range(self.num_classes)}
                # frame_weight.update({x.item():y.item() for x,y in zip(unique_elements, counts_normalized_reverse)})
                
                # # calculate action weight
                # unique_elements, counts = torch.unique(torch.cat(gt_labels), return_counts=True)
                # counts_normalized_reverse = 1 / counts
                # action_weight = {i:1 for i in range(self.num_classes)}
                # action_weight.update({x.item():y.item() for x,y in zip(unique_elements, counts_normalized_reverse)})

                # weight_normalized = torch.tensor(list(frame_weight.values())) * torch.tensor(list(action_weight.values()))
                # weight_normalized /= sum(weight_normalized)
            else:
                weight_normalized = torch.ones(self.num_classes)

            class_weights = weight_normalized.float().cuda()
            # if self.active_learning_method == 'uniform':
            #     class_weights = weight_normalized.float().cuda()
            # elif self.active_learning_method == 'adaptive':
            #     try:
            #         try:
            #             from libs.datasets.finegym_adaptive import CONFUSE_SCORE
            #         except ImportError:
            #             from libs.datasets.thumos_adaptive import CONFUSE_SCORE
            #     except:
            #         from libs.datasets.finegym_adaptive_v2 import CONFUSE_SCORE
            #         # print('Using v2')
            #     class_weights = (torch.tensor(CONFUSE_SCORE).float() * weight_normalized.float()).cuda()
            
            cls_loss = weighted_multilabel_loss(
                torch.cat(out_cls_logits, dim=1)[valid_mask],
                gt_target,
